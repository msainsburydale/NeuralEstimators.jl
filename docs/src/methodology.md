...
```math
  \boldsymbol{\kappa}^* \equiv \underset{\boldsymbol{\kappa}}{\mathrm{arg\,min\;}} \textrm{KL}\{p(\boldsymbol{\theta} \mid \boldsymbol{Z}) \, \| \, q(\boldsymbol{\theta} ;\boldsymbol{\kappa})\}. 
```
...
c^*(\cdot, \cdot) 
\equiv
\underset{c(\cdot, \cdot)}{\mathrm{arg\,min}} \sum_{y\in\{0, 1\}} \textrm{Pr}(Y = y)  \int_\Theta\int_\mathcal{Z}p(\boldsymbol{Z}, \boldsymbol{\theta} \mid Y = y)L_{\textrm{BCE}}\{y, c(\boldsymbol{Z}, \boldsymbol{\theta})\}\textrm{d} \boldsymbol{Z} \textrm{d} \boldsymbol{\theta},
```
