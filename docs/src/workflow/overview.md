...
1. Design and initialise a suitable neural network that maps data to $\mathbb{R}^{d^*}$ (i.e., learned summary statistics of user-specified dimension $d^*$). The architecture class (e.g., MLP, CNN, GNN, DeepSet) should reflect structure of the data (e.g., unstructured, grid, graph, replicated), and any [Flux](https://fluxml.ai/Flux.jl/stable/) model can be used. Given $K$ data sets stored appropriately (see Step 2), the neural network should output a $d^* \times K$ matrix. 
