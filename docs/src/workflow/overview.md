...
1. If constructing a neural posterior estimator, choose a [parameteric family of distributions](@ref "Approximate distributions") used to approximate the posterior distribution. 
1. Design and initialise a suitable neural network. The architecture class (e.g., MLP, CNN, GNN) should align with the multivariate structure of the data (e.g., unstructured, grid, graph). The specific input and output spaces depend on the chosen inferential method: 
    ...
    * For neural posterior estimators, the neural network is a mapping $\mathcal{Z}\to\mathbb{R}^{d^*}$, where $d^*$ is a user-specified number of summary statistics. 
