...
1. Construct a neural estimator by wrapping the neural network in the subtype corresponding to the intended inferential method ([`PointEstimator`](@ref), [`PosteriorEstimator`](@ref), [`RatioEstimator`](@ref)).
1. Train the estimator using [`train`](@ref) and the training set, monitoring performance and convergence using the validation set. 
1. Assess the estimator using [`assess`](@ref) and the test set.
1. Apply the estimator to observed data (see [Inference with observed data](@ref "Inference with observed data")).
