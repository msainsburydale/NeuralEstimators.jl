...
4. Construct a neural estimator by wrapping the neural network in the subtype corresponding to the intended inferential method ([`PointEstimator`](@ref), [`PosteriorEstimator`](@ref), [`RatioEstimator`](@ref)).
...
7. Apply the estimator to observed data (see [Inference with observed data](@ref "Inference with observed data")).
