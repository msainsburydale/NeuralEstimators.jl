...
sampleposterior(q::ApproximateDistribution, tz::AbstractVector, N::Integer; kwargs...) =
    sampleposterior(q, reshape(tz, :, 1), N; kwargs...)
...
numdistributionalparams(layer::CouplingLayer) =
    numdistributionalparams(layer.block1) + numdistributionalparams(layer.block2)
...
function NormalisingFlow(
    ...
    layers = [
        CouplingLayer(d, dstar; use_act_norm = use_act_norm, kwargs...) for
        _ = 1:num_coupling_layers
    ]
...
function logdensity(flow::NormalisingFlow, θ::AbstractMatrix, tz::AbstractMatrix)
    ...
    log_densities =
        -0.5f0 * d * Float32(log(2π)) .- 0.5f0 * sum(U .* U; dims = 1) .+ log_det_J #NB log_det_J must be a 1xK matrix
