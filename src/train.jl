...
function _train(estimator, sampler, simulator;
    ...
    K::Integer = 10_000,
    K_val::Integer = K ÷ 5 + 1,
    lr_schedule::Union{Nothing, ParameterSchedulers.AbstractSchedule} = CosAnneal(
        findlr(optimiser),
        zero(findlr(optimiser)),
        epochs,
        false
    )
)
    ...
    train_time = @elapsed for epoch ∈ 1:epochs
        if store_entire_train_set
            ...
            if epoch == 1 || (epoch % epochs_per_Z_refresh) == 0
                ...
                @sync gc()
                t = @elapsed train_set =
                    _constructset(estimator, simulator, θ_train, m, batchsize)
                verbose && println(" Finished in $(round(t, digits = 3)) seconds")
            end

            # For each batch, update estimator and compute the training risk
            epoch_time = @elapsed train_risk =
                _risk(estimator, loss, train_set, device, optimiser)
        ...
        verbose &&
            println("Epoch: $epoch  Training risk: $(round(train_risk, digits = 3))  Validation risk: $(round(val_risk, digits = 3))  Learning rate: $(@sprintf "%.2E" findlr(optimiser))  Epoch time: $(round(epoch_time, digits = 3)) seconds")
        ...
        if val_risk <= min_val_risk
            ...
            early_stopping_counter > stopping_epochs && verbose &&
                (
    ...
    !isnothing(savepath) &&
        _saveinfo(loss_per_epoch, train_time, savepath, verbose = verbose)
    ...
    train_time = @elapsed for epoch = 1:epochs
        ...
        if store_entire_train_set
            ...
            if epoch == 1 || (epoch % epochs_per_Z_refresh) == 0
                ...
                train_set = nothing
                @sync gc()
                sim_time = @elapsed train_set =
                    _constructset(estimator, simulator, θ_train, m, batchsize)
                verbose &&
                    println(" Finished in $(round(sim_time, digits = 3)) seconds")
            end
            ...
            for θ ∈ _ParameterLoader(θ_train, batchsize = batchsize)
                sim_time +=
                    @elapsed set = _constructset(estimator, simulator, θ, m, batchsize)
                epoch_time +=
                    @elapsed rsk = _risk(estimator, loss, set, device, optimiser)
                push!(train_risk, rsk)
            end
            verbose &&
                println("Total time spent simulating data: $(round(sim_time, digits = 3)) seconds")
            train_risk = mean(train_risk)
        end
        epoch_time += sim_time
...
function _train(estimator, θ_train::P, θ_val::P, Z_train::T, Z_val::T;
    ...
    train_time = @elapsed for epoch = 1:epochs
        ...
        epoch_time =
            @elapsed train_risk = _risk(estimator, loss, train_set, device, optimiser)
        epoch_time += @elapsed val_risk = _risk(estimator, loss, val_set, device)
        loss_per_epoch = vcat(loss_per_epoch, [train_risk val_risk])
        verbose &&
            println("Epoch: $epoch  Training risk: $(round(train_risk, digits = 3))  Validation risk: $(round(val_risk, digits = 3))  Learning rate: $(@sprintf "%.2E" findlr(optimiser))  Epoch time: $(round(epoch_time, digits = 3)) seconds")
...
function train(
    estimator::Union{IntervalEstimator, QuantileEstimatorDiscrete},
    args...;
    kwargs...
)
...
function _constructset(
    estimator::QuantileEstimatorDiscrete,
    Z,
    θ::P,
    batchsize
) where {P <: Union{AbstractMatrix, ParameterConfigurations}}
...
function _risk(estimator, loss, set::DataLoader, device, optimiser = nothing)
    ...
    for (input, output) in set
        ...
        if !isnothing(optimiser)
            ...
            ls, ∇ =
                Flux.withgradient(estimator -> loss(estimator(input), output), estimator)
...
function _risk(
    ...
    for (input, output) in set
        ...
        k = size(output)[end]
        if !isnothing(optimiser)
            ls, ∇ = Flux.withgradient(
                estimator ->
                    Flux.logitbinarycrossentropy(estimator.network(input), output),
                estimator
            )
            Flux.update!(optimiser, estimator, ∇[1])
        else
...
function _risk(
    ...
    for (input, output) in set
        ...
        if !isnothing(optimiser)
            ls, ∇ = Flux.withgradient(
                estimator -> quantileloss(estimator(input), output, τ),
                estimator
            )
            Flux.update!(optimiser, estimator, ∇[1])
        else
...
function _trainmultiple(
    ...
    for i ∈ eachindex(estimators)
        ...
        elseif !isnothing(simulator)
            estimators[i] =
                train(estimators[i], θ_train, θ_val, simulator; m = mᵢ, kwargs...)
        else
...
trainmultiple(estimator, sampler, simulator, M; args...) =
    _trainmultiple(estimator, sampler = sampler, simulator = simulator, M = M; args...)
trainmultiple(
    estimator,
    θ_train::P,
    θ_val::P,
    ...
    M;
    args...
) where {P <: Union{AbstractMatrix, ParameterConfigurations}} = _trainmultiple(
    estimator,
    θ_train = θ_train,
    θ_val = θ_val,
    simulator = simulator,
...
function trainmultiple(
    ...
    M::Vector{I};
    args...
) where {T, P <: Union{AbstractMatrix, ParameterConfigurations}, I <: Integer}
...
function _saveinfo(loss_per_epoch, train_time, savepath::String; verbose::Bool = true)
    ...
    CSV.write(
        joinpath(savepath, "loss_per_epoch.csv"),
        Tables.table(loss_per_epoch),
        header = false
    )
    CSV.write(
        joinpath(savepath, "train_time.csv"),
        Tables.table([train_time]),
        header = false
    )
end
...
function _savebestmodel(path::String)
    ...
    loss_per_epoch =
        load(joinpath(path, "loss_per_epoch.bson"), @__MODULE__)[:loss_per_epoch]
