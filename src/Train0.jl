# NB The simulator method will be the default and the most flexible, so I should focus my efforts on it.
...
function train(θ̂, Ω, P;
	...
	@assert epochs_per_Z_refresh % epochs_per_θ_refresh == 0 "`epochs_per_Z_refresh` should be a multiple of `epochs_per_θ_refresh`"
