...
reshapeZ(Z::V) where {V <: AbstractVector{A}} where {A <: AbstractArray} =
    stackarrays(reshapeZ.(Z))
...
function reshapeZ(Z::V) where {V <:
                               AbstractVector{M}} where {M <: AbstractMatrix{T}} where {T}
...
Base.show(io::IO, D::GNNSummary) = print(
    io,
...
function adjacencymatrix(
    ...
    k::Integer;
    random::Bool = true
) where {Mat <: AbstractMatrix{T}} where {T, F <: AbstractFloat}
    @assert k > 0
    @assert r > 0
...
adjacencymatrix(
    ...
    k::Integer,
    r::F
) where {Mat <: AbstractMatrix{T}} where {T, F <: AbstractFloat} = adjacencymatrix(M, r, k)
...
function adjacencymatrix(
    ...
    moralise::Bool = false,
    combined::Bool = false
) where {Mat <: AbstractMatrix{T}} where {T}
    @assert k > 0
...
function findorderednn(S, k::Integer)
    ...
    while length(query_inds) > 0
        ...
        less_than_l =
            hcat([NN[l, :] .<= query_inds[l] for l ∈ 1:size(NN, 1)]...) |> permutedims
        sum_less_than_l = vec(mapslices(sum, less_than_l, dims = 2))
        ind_less_than_l = findall(sum_less_than_l .>= k+1)
        NN_k =
            hcat([NN[l, :][less_than_l[l, :]][1:(k + 1)] for l ∈ ind_less_than_l]...) |>
            permutedims
