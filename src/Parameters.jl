...
Base.show(io::IO, parameters::P) where {P <: ParameterConfigurations} = print(
    io,
...
Base.show(io::IO, m::MIME"text/plain", parameters::P) where {P <: ParameterConfigurations} =
    print(io, parameters)

size(parameters::P) where {P <: ParameterConfigurations} = size(_extractθ(parameters))
size(parameters::P, d::Integer) where {P <: ParameterConfigurations} =
    size(_extractθ(parameters), d)
...
subsetparameters(θ::P, indices::Integer) where {P <: ParameterConfigurations} =
    subsetparameters(θ, indices:indices)
subsetparameters(θ::M, indices::Integer) where {M <: AbstractMatrix} =
    subsetparameters(θ, indices:indices)
