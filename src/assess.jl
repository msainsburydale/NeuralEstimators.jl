...
function assess(
    ...
    if !isnothing(probs)
        ...
        estimate_names =
            repeat(parameter_names, outer = 2) .* repeat(["_lower", "_upper"], inner = d)
...
function assess(
    ...
    df = _merge2(θ, θ̂)
    probs =
        estimator isa Ensemble{<:IntervalEstimator} ? estimator[1].probs : estimator.probs
    df[:, "α"] .= 1 - (probs[2] - probs[1])
...
function assess(
    estimator::Union{
        ...
        Ensemble{<:QuantileEstimatorContinuous},
        Ensemble{<:QuantileEstimatorDiscrete}
    },
    θ::P, Z;
    parameter_names::Vector{String} = ["θ$i" for i ∈ 1:size(θ, 1)],
    estimator_name::Union{Nothing, String} = nothing,
    ...
    if estimator isa Union{QuantileEstimatorDiscrete, Ensemble{<:QuantileEstimatorDiscrete}}
        probs =
            estimator isa Ensemble{<:QuantileEstimatorDiscrete} ? estimator[1].probs :
            estimator.probs
    else
    ...
    if isnothing(i)
        if estimator isa
           Union{QuantileEstimatorDiscrete, Ensemble{<:QuantileEstimatorDiscrete}}
            set_info = nothing
        else
            set_info = τ
        end
...
function assess(
    ...
    if E == 2 &&
       any(isa.(estimators, Union{IntervalEstimator, Ensemble{<:IntervalEstimator}})) &&
       any(isa.(estimators, Union{PointEstimator, Ensemble{<:PointEstimator}}))
...
function _merge(θ, θ̂)
    ...
    θ̂ =
        stack(θ̂, Not(non_measure_vars), variable_name = :parameter, value_name = :estimate)
...
function risk(df::DataFrame;
    ...
    df = combine(
        df,
        [:estimate, :truth] => ((x, y) -> loss.(x, y)) => :loss,
        ungroup = false
    )
...
function intervalscore(assessment::Assessment;
    ...
    upper = df[:, :upper]
    df[:, :interval_score] =
        (upper - lower) + (2/α) * (lower - truth) * (truth < lower) +
        (2/α) * (truth - upper) * (truth > upper)
    df = groupby(df, grouping_variables)
