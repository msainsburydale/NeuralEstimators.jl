...
function plot(assessment::Assessment; grid::Bool = false) 
	# TODO document this method of plot() and the argument grid 
...
# function _plot_joint(assessment::Assessment)

# 	df = assessment.df
# 	num_estimators = "estimator" ∉ names(df) ? 1 : length(unique(df.estimator))
# 	parameter_names = unique(df.parameter)
# 	parameter_names = Symbol.(parameter_names)
# 	num_params = length(parameter_names)
# 	@assert num_params == 2
# 	df[:, :diff] = df[:, :estimate] -  df[:, :truth]
# 	df = unstack(df, Not(:parameter, :estimate, :truth, :diff), :parameter, :diff)

# 	if num_estimators > 1
# 		colors = [unique(df.estimator)[i] => ColorSchemes.Set1_4.colors[i] for i ∈ 1:num_estimators]
# 		figure = data(df) * mapping(parameter_names[1], parameter_names[2], color = :estimator, col = :estimator) * visual(palettes=(color=colors,), alpha = 0.5)
# 	else
# 		figure = data(df) * mapping(parameter_names[1], parameter_names[2]) * visual(color = :black, alpha = 0.5)
# 	end

# 	# figure += mapping([0], [0]) * visual(ABLines, color=:red, linestyle=:dash) 
# 	figure += mapping([0]) * visual(VLines, color=:red, linestyle=:dash) 
# 	figure += mapping([0]) * visual(HLines, color=:red, linestyle=:dash) 
# 	draw(
# 		figure, 
# 		facet=(; linkxaxes=:minimal, linkyaxes=:minimal), 
# 		axis = (ylabel = "$(parameter_names[2])̂ - $(parameter_names[2])", xlabel = "$(parameter_names[1])̂ - $(parameter_names[1])")
# 		) 
# end
