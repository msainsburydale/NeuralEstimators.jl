...
# Replicated unstructured data {#Replicated-unstructured-data}
...
We first define a function to sample parameters from the prior distribution. Here, we store the parameters as a [`NamedMatrix`](/API/parametersdata#NamedArrays.NamedMatrix) so that parameter estimates are automatically labelled, though this is not required:
...
Next, we train the estimator using [`train`](/API/training#NeuralEstimators.train). Below, we pass our user-defined functions for sampling parameters and simulating data, but one may also pass fixed parameter and/or data instances:
