...
using NeuralEstimators: _runondevice, _check_sizes, _extractθ, nested_eltype, rowwisenorm,
    triangularnumber, forward, inverse
...
@testset "Model-specific functions" begin
    ...
    @testset "Densities" begin
        ...
        function finitedifference(z₁, z₂, ψ, ϵ = 0.0001)
            (_schlatherbivariatecdf(z₁ + ϵ, z₂ + ϵ, ψ) -
             _schlatherbivariatecdf(z₁ - ϵ, z₂ + ϵ, ψ) -
             _schlatherbivariatecdf(z₁ + ϵ, z₂ - ϵ, ψ) +
             _schlatherbivariatecdf(z₁ - ϵ, z₂ - ϵ, ψ)) / (4 * ϵ^2)
        end
        function finitedifference_check(z₁, z₂, ψ)
            @test abs(finitedifference(z₁, z₂, ψ) -
                      schlatherbivariatedensity(z₁, z₂, ψ; logdensity = false)) < 0.0001
        end
        ...
        # Other small tests
        @test schlatherbivariatedensity(3.3, 3.8, 0.9; logdensity = false) ≈
              exp(schlatherbivariatedensity(3.3, 3.8, 0.9))
        y = [0.2, 0.4, 0.3]
        n = length(y)
...
@testset "Layers: $dvc" for dvc ∈ devices
    ...
    @testset "Spatial weight functions: $Weights" for Weights ∈
                                                      [IndicatorWeights, KernelWeights]
        n = 30
        h = rand(1, n)
        n_bins = 10
        w = Weights(1, n_bins)
        ...
        for dₓ in (0, 2) # dimension of set-level inputs 
            for data in ("unstructured", "grid", "graph")
                ...
                elseif data == "graph"
                    ...
                    propagation =
                        GNNChain(SpatialGraphConv(1 => 16), SpatialGraphConv(16 => dₜ))
                    readout = GlobalPool(mean)
                    ψ = GNNSummary(propagation, readout)
                end
                ...
                train(
                    ...
                    batchsize = length(M),
                    verbose = verbose
                ) # NB move ds to CPU to check that an estimator passed to train() on the CPU can be trained on the GPU
            end
        end
    end
end
...
simulatearray(parameters::Parameters, m) =
    [θ[1] .+ θ[2] .* randn(Float32, n, m) for θ ∈ eachcol(parameters.θ)]
            ...
            @testset "assess" begin
                ...
                if covar == "set-level covariates"
                    ...
                    coverage(
                        ...
                        average_over_sample_sizes = false
                    )
...
@testset "initialise_estimator" begin
    ...
    @test typeof(initialise_estimator(
        p,
        architecture = "MLP",
    ...
    @test_throws Exception initialise_estimator(
        p,
        architecture = "CNN",
...
@testset "Ensemble: $dvc" for dvc ∈ devices
    ...
    ensemble = train(
        ...
        verbose = verbose,
        use_gpu = dvc == gpu,
        optimiser = Flux.setup(Adam(5e-3), ensemble)
    )
...
@testset "EM" begin
    ...
    function simulateconditional(
        ...
        ξ;
        nsims::Integer = 1
    ) where {M <: AbstractMatrix{Union{Missing, T}}} where {T}
    ...
    neuralMAPestimator = initialise_estimator(
        p,
        architecture = "CNN",
        kernel_size = [(10, 10), (5, 5), (3, 3)],
        activation_output = exp
    )
    ...
    Z = simulate(θ, 1)[1]# simulate a single gridded field
    @test_warn "Data has been passed to the EM algorithm that contains no missing elements... the MAP estimator will be applied directly to the data" neuralem(
        Z,
        θ₀,
        ξ = ξ,
        nsims = H
    ...
    Z = Z[:, :]
    Z = removedata(Z, 1.0)
    @test_throws Exception neuralem(
        Z,
        θ₀,
        ξ = ξ,
        nsims = H,
        use_ξ_in_simulateconditional = true
    )
